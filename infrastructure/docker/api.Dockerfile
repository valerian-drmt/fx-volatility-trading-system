...
# pyproject.toml + src/ are needed together because PEP 517 build resolves
# the package layout from src/ when installing fxvol[api]. R9 src-layout:
# api, bus, core, engines, persistence and shared all live under src/, so a
# single copy ships the whole runtime. Keep src/ explicit (not COPY . .) so
# we do not ship notebooks, tests, docs.
