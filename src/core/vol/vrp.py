...
- ``VRP_DEFAULTS_VOL_PTS`` : tabulated VRP per (regime, tenor). Single source
  of truth, consumed by cockpit health metrics and the vol-engine regime gate
  (the static ``vrp_default_curve`` mirror table was dropped in migration 037).
...
# cockpit (read-only health) and the vol-engine regime gate.
