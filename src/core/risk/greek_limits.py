...
def compute_caps(nav_base: float, spot: float, regime_mult: float = 1.0) -> GreekCaps:
    ...
    At nav_base=812_000, spot=1.08, regime_mult=1 → delta≈$243.6k, vega≈$5,075,
    gamma≈$3,007/pip (spec §2 sanity values).
    """
    if nav_base <= 0 or spot <= 0 or regime_mult <= 0:
    ...
    return GreekCaps(
        delta_usd=BETA["delta"] * loss / s,
        vega_usd=BETA["vega"] * loss / v,
        gamma_pip=2 * BETA["gamma"] * loss / (s**2 * spot * 1e4),
        cross_usd=BETA["cross"] * loss,
