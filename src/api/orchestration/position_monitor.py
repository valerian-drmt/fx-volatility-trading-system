...
class PositionMonitorScheduler:
    ...
    async def _monitor_one(
        ...
        # Compute signal tracking cols (folded into the same mtm row since
        # migration 026 / Theme 2). All NULL for positions not opened from
        # a triggering PCA signal.
        ...
        if triggering_pc is not None and entry_z is not None:
            ...
            if current is not None:
                ...
                signal_cols.update({
                    ...
                    "weakening_ratio": ratio,
                    "sign_flipped": flipped,
                    "signal_status": status,
                })

        # Persist mtm + signal in a single row (folded since Theme 2).
