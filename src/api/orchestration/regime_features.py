...
def _lookup_regime(pattern: str) -> dict[str, Any]:
    ...
    ``core.regime_patterns.REGIME_PATTERNS`` (was the ``regime_pattern_dict``
    DB mirror until migration 042 dropped it). Always returns a row —
    falls back to ``unmapped_extreme`` when the pattern is unseen.
    """
