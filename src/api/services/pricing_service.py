"""Wrap core.pricing.bs — converts API request models to BS primitives + IV root-find."""
...
# Convert incoming "CALL"/"PUT" API tokens to bs.* "C"/"P" convention.
_RIGHT_MAP: dict[str, str] = {"CALL": "C", "PUT": "P"}


def _T_years(maturity_days: int) -> float:
    """Actuarial-style annualization (365 days, consistent with bs.theta)."""
    return maturity_days / 365.0


def compute_price(req: PriceRequest) -> PriceResponse:
    right = _RIGHT_MAP[req.option_type]
    price = bs.bs_price(req.spot, req.strike, _T_years(req.maturity_days),
...
def compute_greeks(req: GreeksRequest) -> GreeksResponse:
    ...
    return GreeksResponse(
        ...
        delta=bs.bs_delta(F, K, T, sigma, right),
        gamma=bs.bs_gamma(F, K, T, sigma),
        vega=bs.bs_vega(F, K, T, sigma),
        theta=bs.bs_theta(F, K, T, sigma, right),
    )
...
def compute_implied_vol(req: ImpliedVolRequest) -> ImpliedVolResponse:
    ...
    def diff(sigma: float) -> float:
        return bs.bs_price(F, K, T, sigma, right) - req.market_price
