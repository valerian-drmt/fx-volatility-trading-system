...
COLUMNS = [
    ...
    "P&L Total", "P&L Δ", "P&L Vega", "P&L Θ", "IV Entry %", "IV Now %",
]
_COL_ID = 0
_COL_SYMBOL = 1
_COL_SIDE = 2
...
_COL_IV_ENTRY = 12
_COL_IV_NOW = 13
_COL_CANCEL = 14  # only for open orders table
...
class OrdersPanel(QWidget):
    ...
    def _extract_order_row(self, order: Any) -> tuple[str, ...]:
        if isinstance(order, dict):
            ...
            pnl_total = self._fmt_pnl(order.get("pnl_total"))
            pnl_delta = self._fmt_pnl(order.get("pnl_delta"))
            pnl_vega = self._fmt_pnl(order.get("pnl_vega"))
            pnl_theta = self._fmt_pnl(order.get("pnl_theta"))
            iv_entry = self._fmt_iv(order.get("iv_entry_pct"))
            iv_now = self._fmt_iv(order.get("iv_now_pct"))
        else:
            ...
            pnl_total = self._fmt_pnl(getattr(order, "pnl_total", None))
            pnl_delta = self._fmt_pnl(getattr(order, "pnl_delta", None))
            pnl_vega = self._fmt_pnl(getattr(order, "pnl_vega", None))
            pnl_theta = self._fmt_pnl(getattr(order, "pnl_theta", None))
            iv_entry = self._fmt_iv(getattr(order, "iv_entry_pct", None))
            iv_now = self._fmt_iv(getattr(order, "iv_now_pct", None))
        return (
            str(order_id), str(symbol), str(side), str(order_type), str(qty), str(price), str(status), str(t),
            pnl_total, pnl_delta, pnl_vega, pnl_theta, iv_entry, iv_now,
        )
