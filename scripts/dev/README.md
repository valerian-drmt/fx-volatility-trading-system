...
| `smoke_r1_p6_indices.py` | Functional smoke test of migration 002_add_indices : 16 expected indices present, GIN on `vol_surfaces.surface_data`, partial WHERE on `positions.status_active`, DESC sort on all timestamp indices, round-trip downgrade/upgrade. | R1 PR #6 |
