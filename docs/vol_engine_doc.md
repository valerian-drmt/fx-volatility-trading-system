...
```
IB Gateway (client_id=2, own connection)
    │
    ▼
Step 1: IV Surface Extraction (in VolEngine._run_scan)
    │  FOP chain scan → BS IV extraction → delta-space interpolation
    │  Output: σ_ATM, RR25, BF25, smile pillars per tenor
    │
    ▼
Step 2: Fair Vol Model (in VolEngine._run_scan)
...
Signal: CHEAP / EXPENSIVE / FAIR per tenor
    │
    ▼
Output → vol_output_queue → controller → vol_scanner, term_structure, smile panels
         + risk_engine.iv_surface (for greeks computation)
```

**Implementation:** `services/vol_engine.py` — Thread 2 (`VolEngine`). Runs every 180s (configurable via `config/vol_config.json`). Not started if market is closed. Config edited via Settings dialog.
...
Per tenor: `{tenor_label, expiry, dte, F, σ_ATM, RR25, BF25, iv_10dp, iv_25dp, iv_25dc, iv_10dc, strike_pillars}` → `vol_output_queue` as `pillar_rows`
