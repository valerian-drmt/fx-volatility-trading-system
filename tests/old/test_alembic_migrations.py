...
# (autogenerated post-R8) drops every index created in 002_add_indices.
# The ``test_all_indices_created`` smoke below therefore only asserts that
# ``alembic upgrade head`` runs to completion — no schema-level invariant
# remains on the legacy index names. Newer indexes created via 020+
# migrations live on their renamed tables and are covered by the
# integration suites under tests/integration/pipeline_db_writer.
