"""Integration tests for /api/v1/admin/config via FastAPI TestClient."""
...
def test_get_current_returns_defaults_on_empty_db(client):
    ...
    assert body["config"]["signal"]["threshold_vol_pts"] == 1.0
...
def test_put_creates_new_version_and_publishes(client):
    ...
    r = tc.put("/api/v1/admin/config", json={
        ...
        "user": "valerian", "comment": "tighter",
    })
    assert r.status_code == 200, r.text
    body = r.json()
    assert body["version"] == 1
    assert body["config"]["signal"]["threshold_vol_pts"] == 2.5
    redis.publish.assert_awaited_once_with("config:changed", "1")
