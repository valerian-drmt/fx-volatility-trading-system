...
def _assert_published(redis_mock: AsyncMock, version: int) -> None:
    redis_mock.publish.assert_awaited_once()
    channel, payload = redis_mock.publish.await_args.args
    assert channel == CONFIG_CHANGED_CHANNEL
    assert f'"version": {version}' in payload
...
async def test_update_inserts_new_version_and_publishes(session, redis_mock):
    ...
    assert rec1.config.signal.z_threshold_arm == 1.0
    assert rec1.config.signal.z_threshold_strong == 2.0  # default kept
    _assert_published(redis_mock, 1)
    ...
    assert rec2.config.signal.z_threshold_arm == 1.0  # preserved from v1
