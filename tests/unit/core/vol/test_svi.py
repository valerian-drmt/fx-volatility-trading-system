"""Tests for ``core.vol.svi`` — raw SVI fit + butterfly no-arbitrage check.
...
then verify ``butterfly_g_min`` discriminates clean vs degenerate smiles.

Resurrected from ``tests/old/sandbox_r9/test_svi_fit.py`` and
``tests/old/sandbox_r9/test_svi_butterfly_arb.py`` (git 14175622~1), merged.
"""
...
from core.vol.svi import SviParams, butterfly_g_min, fit_svi, svi_curve, svi_iv
