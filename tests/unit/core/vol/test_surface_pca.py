...
Resurrected from ``tests/old/sandbox_r9/test_surface_pca.py`` (git 14175622~1);
the previously unseeded ``np.random.rand()`` in the level-factor test now
uses a seeded generator to kill flake risk.
"""
...
def test_pca_fit_level_factor_dominates_when_only_level_varies() -> None:
    # 60 surfaces differing only in ATM level (seeded — was np.random.rand()).
    rng = np.random.default_rng(0)
    surfaces = [_surface(atm_level=0.05 + 0.01 * rng.random()) for _ in range(60)]
