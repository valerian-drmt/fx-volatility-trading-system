...
Resurrected from ``tests/old/sandbox_r9/test_ssvi.py`` (git 14175622~1),
extended with an explicit calendar no-arbitrage (total-variance
monotonicity) check and a flag-consistency check.
"""
...
from core.vol.ssvi import fit_ssvi, ssvi_curve_for_tenor, ssvi_iv, ssvi_total_variance
...
def test_ssvi_calendar_total_variance_monotone_in_theta() -> None:
    """No calendar arbitrage ⇔ total variance non-decreasing in T at fixed k.

    Fit the synthetic surface, then evaluate ``w(k, θ(T))`` on an increasing
    θ grid built from a 1M→6M non-decreasing ATM term structure and assert
    monotonicity at every fixed log-moneyness.
    """
    obs, atm = _synthetic_surface()
    fitted = fit_ssvi(obs, forward=1.17, atm_iv_by_tenor_years=atm)
    assert fitted is not None
    eta, gamma, rho = fitted["eta"], fitted["gamma"], fitted["rho"]

    tenors = np.arange(1, 7) / 12.0                 # 1M .. 6M
    atm_ts = 0.06 + 0.002 * np.arange(6)            # non-decreasing ATM vols
    theta = atm_ts * atm_ts * tenors                # increasing total ATM variance

    for k in (-0.05, -0.02, 0.0, 0.02, 0.05):
        w = ssvi_total_variance(np.full_like(theta, k), theta, eta, gamma, rho)
        assert np.all(np.diff(w) >= -1e-10), f"calendar arb at k={k}: w={w}"


def test_ssvi_calendar_flag_matches_condition_on_returned_params() -> None:
    """``calendar_arb_free`` must equal ``2γ ≥ 1 − ρ²`` recomputed from the
    returned params — the fit's bounds keep the result in the safe region,
    and the flag must faithfully report the condition (not be hardcoded).
    """
    obs, atm = _synthetic_surface()
    fitted = fit_ssvi(obs, forward=1.17, atm_iv_by_tenor_years=atm)
    assert fitted is not None
    gamma, rho = fitted["gamma"], fitted["rho"]
    condition = (2.0 * gamma) >= (1.0 - rho * rho) - 1e-6
    assert fitted["calendar_arb_free"] is bool(condition)
