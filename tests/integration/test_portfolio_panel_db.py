...
The DB session is created via an ``@asynccontextmanager`` rather than a pytest
async fixture on purpose: the pinned pytest-asyncio (0.23.x) can't wrap async
fixtures on pytest 9 — but async test functions work fine.
"""
...
async def test_equity_curve_distinct_on_keeps_latest_per_bucket():
    ...
    async with _live_session() as db:
        ...
        await _snap(db, base + timedelta(seconds=10), 100_000)
        await _snap(db, base + timedelta(seconds=40), 111_111)          # latest in bucket 0
        await _snap(db, base + timedelta(minutes=1, seconds=5), 222_222)  # bucket 1

        points = await equity_curve(db, window="1d")
...
async def test_equity_curve_excludes_rows_outside_window():
    """Rows older than the window's lookback are filtered out server-side."""
    from api.routers.portfolio_panel import equity_curve

    async with _live_session() as db:
        await _snap(db, datetime.now(UTC) - timedelta(days=10), 100_000)
        points = await equity_curve(db, window="1d")

    assert points == []


async def test_equity_curve_flags_eod_only_before_22_utc():
    ...
    async with _live_session() as db:
        await _snap(db, t21, 100_000)
        await _snap(db, t23, 200_000)
        points = await equity_curve(db, window="30d")

    is_eod = {p["net_liq_usd"]: p["is_eod"] for p in points}
